...
bsdb_PD_signatures_allMethods <- mutate(bsdb_PD_signatures_allMethods,
                                        `Body site` = ifelse(`Body site` == "Feces,Blood", "Blood,Feces"))
...
studies_per_year <- bsdb_PD_signatures_allMethods %>% select(PMID, Year) %>% distinct() %>% .$Year %>% table() %>% as.data.frame() %>% arrange(desc(Freq)) %>% set_colnames(c("Year", "N. Occurrences")) 

years_barplot <- ggplot(studies_per_year, aes(x = Year, y = `N. Occurrences`)) + 
  geom_col(fill = "#34a270") + 
  geom_text(data = studies_per_year, mapping = aes(x = Year, y = `N. Occurrences` + 0.2, label = `N. Occurrences`)) + 
  theme_light() +
...
bodySites <- bsdb_PD_signatures_allMethods %>% select(PMID, `Body site`) %>% distinct() %>% mutate(`Body site` = ifelse(`Body site` == "Feces,Blood", "Blood,Feces", `Body site`)) %>% .$`Body site` %>% table() %>% as.data.frame() %>% arrange(desc(Freq)) %>% set_colnames(c("Body Site", "N. Occurrences")) 

bodySite_barplot <- ggplot(bodySites, aes(x = reorder(`Body Site`, - `N. Occurrences`), y = `N. Occurrences`)) + 
  geom_col(fill = "#cf2fb3") + 
  geom_text(data = bodySites, mapping = aes(x = reorder(`Body Site`, - `N. Occurrences`), y = `N. Occurrences` + 1.5, label = `N. Occurrences`)) + 
  theme_light() +
...
censusMethods <- bsdb_PD_signatures_allMethods %>% select(PMID, `Sequencing type`) %>% distinct() %>% .$`Sequencing type` %>% table() %>% as.data.frame() %>% arrange(desc(Freq)) %>% set_colnames(c("Census Method", "N. Occurrences")) 

censusMethods_barplot <- ggplot(censusMethods, aes(x = reorder(`Census Method`, - `N. Occurrences`), y = `N. Occurrences`)) + 
  geom_col(fill = "#0c8dc3") + 
  geom_text(data = censusMethods, mapping = aes(x = reorder(`Census Method`, - `N. Occurrences`), y = `N. Occurrences` + 1, label = `N. Occurrences`)) + 
  theme_light() +
...
ggarrange(
  ggarrange(years_barplot, censusMethods_barplot,  nrow = 2), 
  bodySite_barplot, 
  ncol = 2
  )
...
```{r}
createTaxonTable(bsdb_PD_signatures_allMethods, Inf) %>% 
...
shotgun_curated_studies_GA <- read_tsv("~/Documents/git_repos/parkinsonsMetagenomicData/parkinson_shotgun_datasets.tsv")
