...
tmp_df <- computeStandardizedMeanDifference(data_okAnalysis.se)  
...
```{r asinh transformation, eval=FALSE}
...
old_vs_new_implementation <- full_join(
  rownames_to_column(tmp_df, "FeatureID") %>% 
    select(FeatureID, RE, p.value = `p-value`) %>% 
    mutate(RE = as.double(RE), p.value = as.double(p.value)),
  meta_analysis %>% select(FeatureID, RE, p.value), by = "FeatureID") %>%
...
I still can't replicate the filter_sort function, need to study further. But a 
workaround makes the function applicable

```{r}
reformatted.df <- meta_analysis_complete %>% 
  column_to_rownames("FeatureID") %>% 
  mutate(CI_RE = paste(CI_lower, CI.upper, sep = ";"), .before = CI_lower) %>% 
  select(-CI_lower, -CI.upper)

```

```{r}
head(arrange(tmp_df, desc(abs(as.numeric(RE)))))
```
