...
taxonomyForestPlot <- biobakeryForestPlot(
...
forest_plot_with_legend <- ggarrange(forest_plot, ggtexttable(legend_table, rows = NULL), nrow = 1, widths = c(1, 0.5))

invisible(sapply(c("png", "svg"), function(fmt) ggsave(plot = forest_plot, filename = sprintf("results/03_meta_analysis_SGB/ForestPlot_%s.%s", params$transform, fmt), dpi = 300, height = 8, width = 8)))
...
# Blue Poo - ongoing harmonization
  ...
  2. Metadata retrieved. useful metadata items are:
    - bowel_movements_last_7_days
    - bristol_stool_score_average_last_3_months
    - transit time (days)
